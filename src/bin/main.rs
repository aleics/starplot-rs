...
impl App {
    ...
    fn render(&mut self, args: &RenderArgs, font_path: &Path) {  
        ...
        let square = graphics::rectangle::square(self.star.x, self.star.y, self.star.size);
        ...
        self.gl.draw(args.viewport(), |c, gl| {
            ...
            // draw ellipse
            graphics::ellipse(BLACK, square, c.transform, gl);    
        });
    }
    ...
    fn on_input(&mut self, inp: Input) -> Option<Action> {
        match inp {
            Input::Release(but) => {
                match but {
